...
with: x-amz-content-sha256: STREAMING-AWS4-HMAC-SHA256-PAYLOAD
...
Usage:
    python3 s3_stream_sha256_payload.py my-bucket us-east-1 test/key.bin 1048576 [--endpoint http://host:port]
...
def aws_chunked_content_length(decoded_len: int, chunk_size: int) -> int:
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload a generated file to S3 using SigV4 aws-chunked streaming with SHA256 payload."
    )
    ...
    decoded_len = size
    payload_hash_header_value = "STREAMING-AWS4-HMAC-SHA256-PAYLOAD"

    content_length = aws_chunked_content_length(decoded_len, chunk_size)
    vlog(
        "prepared request "
    ...
    try:
        ...
        with open(path, "rb") as f:
            while True:
                ...
                chunk_data_hash_hex = sha256_hex(data)
        ...
        if not status_line.startswith("HTTP/1.1 2") and not status_line.startswith("HTTP/1.0 2"):
            sys.exit(1)
    finally:
        sock.close()
